...
if (isset($_POST['simpan'])) {
    ...
    if ($namawisata == '' || $alamatwisata == '' || $deskripsiwisata == '' || $namakuliner == '' || $alamatkuliner == '' || $deskripsikuliner == '') {
        ...
        if ($id == "") {
            $sql = "INSERT INTO papua (namawisata, namakuliner, alamatwisata, alamatkuliner, deskripsiwisata, deskripsikuliner, foto1, foto2, foto3, foto4, foto5, foto6) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
            ...
            $sql = "UPDATE papua SET namawisata=?, namakuliner=?, alamatwisata=?, alamatkuliner=?, deskripsiwisata=?, deskripsikuliner=?, foto1=?, foto2=?, foto3=?, foto4=?, foto5=?, foto6=? WHERE id=?";
            $stmt = mysqli_prepare($koneksi, $sql);
            if ($stmt === false) {
                $error = "Error: " . mysqli_error($koneksi);
            } else {
...
<body>
    ...
    <div class="mb-3 row">
        <a href="lokasi_papua.php">Kembali ke halaman admin tempat</a>
    </div>
